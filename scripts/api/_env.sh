...
#
# ⚠ BASH ONLY. The repo root is resolved from $BASH_SOURCE, which zsh does not
# set, so sourcing this from a zsh prompt looks for .env one directory up from
# wherever you are and fails with a confusing "no such file or directory".
# Wrap it: bash -c 'source scripts/api/_env.sh && ...'
#
...
if [ -z "${SIGMA_API_TOKEN:-}" ]; then
  ...
  if $_fresh; then
    ...
    # The in-repo fetcher, so a plain `git clone` works with nothing installed.
    # $SIGMA_TOKEN_FETCHER overrides it if you have your own auth flow.
    ...
    if [ ! -f "$_gettoken" ]; then
      ...
      echo "  Expected scripts/api/get-token.sh in the repo, or set" >&2
      echo "  SIGMA_TOKEN_FETCHER to a script that prints:" >&2
      echo "    export SIGMA_API_TOKEN=<token>" >&2
