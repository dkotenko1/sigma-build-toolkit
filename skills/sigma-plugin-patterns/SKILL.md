...
description: >-
  Proven architectural patterns and recipes for Sigma Computing plugins:
  the JSON-settings pattern for configuration richer than the editor panel
  allows, plus reusable flows for state, config and interaction. Use alongside
  sigma-plugin-development when structuring a plugin — this covers how to
  organize one, that covers the SDK surface.
