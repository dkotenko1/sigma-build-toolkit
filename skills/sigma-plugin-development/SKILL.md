...
description: >-
  Reference for building a Sigma Computing plugin with the
  @sigmacomputing/plugin SDK — editor-panel configuration, reading element data,
  control variables, action triggers and effects, environment/style/lifecycle,
  registration and hosting. Use when building, configuring, or debugging a plugin
  itself (React, Vue, Svelte, or vanilla JS), typically because a visualization
  has no native Sigma element equivalent. Pair with sigma-plugin-patterns for
