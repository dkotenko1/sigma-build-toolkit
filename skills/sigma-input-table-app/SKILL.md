...
description: >-
  Build an interactive Sigma "data app" from code (POST/PUT
  /v2/workbooks/spec) — scenario modelers, revenue/demand forecasting,
  budgeting and planning, write-back tools, and adjustment + change-log apps
  (users edit values through a modal, every change logged). Trigger when users
  need to CREATE scenarios, ENTER or adjust forecasts, model against a baseline,
  or "update values via a modal / add an adjuster / build a planning app".
  Encodes the verified input-table, linked-input-table, modal, button-action and
  control-driven-formula shapes, the hard limits, and the non-negotiable defaults
  (delta/variance columns, comparative KPIs, sensible number formatting, charts
  parented to the linked input table). Build from scratch — never re-POST a
  UI-built workbook's GET-back. For population segmentation instead of numeric
  projection, use sigma-cohort-builder-app.
...
> reference/scenario-modeler-pattern.md (see the Scenario Modeler section below) —
...
and add a read-only dashboard page of your own for the overview half.

## The modeler completeness checklist — grade the build against this

The command-center equivalent is
`../sigma-workbook-conventions/reference/command-center-recipes.md` §0. Same
failure mode here: a modeler that has a grid, three KPIs and a bar chart is
*correct* and still half a product. A finished one has all seven rows.

| Block | Done means |
|---|---|
| Editable grid | drivers + a **live Base Case** + Δ column + entered-cell tint |
| Comparative KPIs | projected vs baseline via `comparisonColumn`, plus uplift % |
| **Named scenarios** | a `Scenarios` input table joined in through a pivot, so a user creates and switches between *several* named scenarios — not one anonymous set of edits |
| **A/B comparison chart** | baseline **and** projected as two series (long format, `stacking:"none"`) — not two separate single-series charts |
| Bulk edits | at least one `update-rows` button (reset, and one opinionated scenario) |
| **Lifecycle** | Create → Submit → Approve, each a modal writing to a status log with `CREATED_AT`/`CREATED_BY` |
| Copilot | a **second agent**, scoped to this page, with tools that set the drivers and create a scenario |

The two most often missing are the **named-scenario dimension** and the
**two-series comparison chart**. Without the first, "modeling" is one set of
edits with nothing to compare against; without the second, the audience has to
hold the baseline in their head. Both shapes are in "The verified foundation"
below (§2–§3 and the long-format note under "Verified shape gotchas").
...
The reference scenario modeler in `examples/build_demand_planning_lite.py` is
...
marketing command-center exemplar in
`../sigma-workbook-styling/examples/marketing-control-center.json`) is the bar for **read-only marketing/exec
...
2. Assets: real logo via `scripts/fetch_logo.py <domain>` (see
   `../sigma-workbook-styling/SKILL.md` for the recolour rule); any hero art as a
   data-URI SVG. Keep API keys for image generation outside the repo and out of
   `/tmp`, which gets pruned.
...
`examples/build_demand_planning_lite.py` — the canonical
...
- `examples/build_demand_planning_lite.py` — a complete write-back app; **the
...
**Quality bar = the `examples/build_demand_planning_lite.py` scenario modeler.** A bare "3 KPIs + a bar + an input table" is
