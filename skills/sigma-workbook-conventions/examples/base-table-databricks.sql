...
--
-- This is DATABRICKS SQL. See base-table-snowflake.sql for the Snowflake
-- version, and reference/warehouse-portability.md for the full dialect mapping.
--
...
months AS (
    ...
    SELECT ADD_MONTHS(DATE_TRUNC('MONTH', CURRENT_DATE()), CAST(id AS INT) - 23) AS period_month,
           CAST(id AS INT) AS month_index
    FROM RANGE(24)
),
grid AS (
    SELECT
        ...
        (MOD(ABS(HASH(CONCAT(s.state, p.product))), 21) - 10) / 100.0 AS state_tilt,
...
SELECT
    ...
    CAST(product_order AS INT)       AS "Product Order",
    CAST(state AS STRING)      AS "State",
    CAST(period_month     AS DATE)         AS "Period",
    CAST(period_name AS STRING)      AS "Period Name",
    ...
    CAST(volume * yield_rate AS DECIMAL(18,4)) AS "Revenue",
    CAST(volume * cost_rate AS DECIMAL(18,4)) AS "Cost",
    CAST(fee_monthly AS DECIMAL(18,4)) AS "Ancillary",
    ...
    CAST(volume * yield_rate - volume * cost_rate + fee_monthly AS DECIMAL(18,4)) AS "Net Revenue",
    CAST(volume * risk_rate AS DECIMAL(18,4)) AS "Provision",
    CAST(volume * yield_rate * opex_ratio AS DECIMAL(18,4)) AS "Opex",
    CAST(volume * yield_rate - volume * cost_rate + fee_monthly
         - volume * risk_rate - volume * yield_rate * opex_ratio AS DECIMAL(18,4)) AS "Contribution Profit",
    CAST(risk_rate AS DECIMAL(9,4))  AS "Risk Rate",
    CAST(cost_rate * 100 AS DECIMAL(9,4))  AS "Cost Pct",
    CAST(goal_pct AS DECIMAL(9,4))  AS "Goal Pct"
FROM calc
