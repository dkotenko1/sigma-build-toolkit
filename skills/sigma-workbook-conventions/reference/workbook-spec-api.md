...
>
> **This file predates the August 2026 schema change in places.** Where you see
> the layout tags `<GridContainer>` and `<LayoutElement>`, the current API wants
> **`<Container>`** and **`<Element>`**. The old tags do not produce a helpful
> error — they return a **masked 400** (`An error has occurred,
> incident-id=…`) that never names the tag, so this is an expensive thing to
> get wrong.
>
> Likewise, elements now live in **one flat `document.elements` list**, not
> nested under `pages[].elements`, and `layout` lives at `document.layout`.
>
> `reference/schema-2026-08-breaking-changes.md` is authoritative on all of
> this. `scripts/validate-spec.py` flags the legacy tags for you.
> Everything else in this file — element shapes, formulas, controls, encodings
> — is unaffected.
...
3. **Container children must be nested inside `<Container>` in the XML.**
   ...
   structure. A `<Container>` with no nested `<Element>` /
   `<Container>` children renders as an empty box, and all the elements
...
These return **200**, produce no error, no empty box and no console warning, and
render as **nothing at all**. Together they cost more debugging time than every
documented error combined, because the natural reaction is "my data is wrong"
when the data is fine.

**1. A container's internal rows must not exceed the span its parent grants.**

```
max(child end row) - 1  <=  (parent end row - parent start row)
```

A container given `gridRow="1 / 6"` grants 5 rows. If its children end at row
11, it needs 10 — and the whole container collapses to zero height. **This is
the most expensive one.** `scripts/validate-spec.py` does not catch it yet;
check it by hand or extend the linter.

**2. A `text` element has no `source` field.** Its fields are `body`, `id`,
`kind`, `overflow`, `verticalAlign`. A `{{formula}}` inside one resolves *only*
against a sourced data element sharing its container. This is why stat cards and
alert tiles carry a small KPI next to the prose — that KPI is not decoration, it
is what gives the text something to bind to. Remove it and every `{{...}}` in
the card silently goes blank.

**3. Overlapping siblings drop silently.** Which one vanishes is not
predictable. `create` reports `Element collisions found` only *sometimes*, so a
clean create does not mean your grid ranges don't overlap.
`scripts/validate-spec.py` catches this statically.

**4. `1fr` row tracks collapse inside an auto-height `<Tab>`.** Use
`gridTemplateRows="auto"` inside tabs. A `repeat(n, 1fr)` there resolves against
a height that isn't known yet, and the contents vanish.

Two related traps worth knowing in the same breath:

- **`progress` loses its binding inside a `<Tab>`** unless it carries an
  explicit `source` — see `command-center-recipes.md` §3.
- **A `/` in an element `name` breaks every `[Element/Column]` reference to
  it.** Sigma parses the first `/` as the element/column separator, so
  `[Buy / Markdown Plan/Revenue]` resolves to garbage and every dependent
  element shows "Invalid Argument". Never put `/` in the name of an element
  other formulas reference.

**The only reliable detection is visual.** Generate → lint → push → export a
PNG → *look at the image* → fix. Four of the failures above are invisible in the
spec and obvious in a screenshot.
...
| `DELETE` | _unknown against staging_ | Against a different org (an Azure-hosted org on another regional API host, 2026-08-14), `DELETE /v2/files/{urlId}` returned `200 {}` and cleanly archived a workbook the same token had just created via `POST /v2/workbooks/spec`. If staging still 404s, it may be host/org-specific rather than universally broken — retry `/v2/files/{urlId}` (not `/v2/workbooks/{id}`) before concluding it's unavailable. |
...
chasing that is a trap: on a real 2026-08-14 debugging session against a
Databricks-backed org it burned a huge amount of time investigating writeback
grants, connection health, malformed columns, and `inputMode` before the actual
cause surfaced.
...
| Combo chart | `combo-chart` | `xAxis: {columnId}`, `yAxis: {columnIds: ["a", {columnId: "b", type: "line"}]}`, optional `yAxis2: {columnIds: [...]}` | **Corrected 2026-08-24.** `yAxis` is an OBJECT with `columnIds` (not a bare list), and a per-series entry keys on **`columnId`**, not `id`. Only `columnIds[0]` can be a bar — every later series is silently rewritten to `line`. See the combo-chart bullet in `schema-2026-08-breaking-changes.md` §11. |
...
one org but not another. So `pivot-table` is not blanket-rejected — something about
...
```json
{
  ...
  "source": {
    ...
    "connectionId": "REPLACE_WITH_YOUR_CONNECTION_ID",
...
## Column `format` — the shape, now known

The error that used to block this is real but the message names the fix:

```
pages[0].elements[N].columns[M].format: Missing "kind" field
```

`format` needs `kind` plus a d3-style `formatString`. These are verified
working at POST and PUT:

```python
MONEY    = {"kind": "number", "formatString": "$,.0f", "currencySymbol": "$"}
MONEY_2DP= {"kind": "number", "formatString": "$,.2f", "currencySymbol": "$",
            "decimalSymbol": ".", "digitGroupingSymbol": ",", "digitGroupingSize": [3]}
MONEY_CMP= {"kind": "number", "formatString": "$,.3s", "currencySymbol": "$"}  # 123092071 -> $123.1M
PCT      = {"kind": "number", "formatString": ".2%"}
PCT_1DP  = {"kind": "number", "formatString": ".1%"}
NUM      = {"kind": "number", "formatString": ",.0f",
            "digitGroupingSymbol": ",", "digitGroupingSize": [3]}
NUM_1DP  = {"kind": "number", "formatString": ",.1f"}
```

Notes:

- `formatString` is d3-format syntax. `%` scales by 100 automatically — do
  **not** also multiply the formula by 100 or you get 4500% instead of 45%.
- `$,.3s` is SI-compact. Reach for it whenever a column sums raw dollars:
  a cohort total rendered as `$93,300,131` under `$,.0f` is technically right
  and reads as noise.
- **Match the format to the magnitude the column is actually in.** A column
  already expressed in millions formatted with a billions format produced
  `$1,050.00` for a trillion-dollar figure — the number was right and read as
  nonsense.
- `currencySymbol` is required alongside a `$` in `formatString`; the symbol in
  the format string alone is not enough on every element kind.
- Formats do **not** always propagate from a data-model metric. Verify after
  CREATE: if a tile shows `12345.67` instead of `$12,345.68`, set `format`
  explicitly on that column.
...
- `<Page>` — the root. `type="grid"`, `gridTemplateColumns="repeat(24, 1fr)"`,
  ...
  `pages` array).
> Tag names below are the current ones. Pre-August-2026 specs use
> `<GridContainer>` / `<LayoutElement>`; those now fail with a masked 400.
