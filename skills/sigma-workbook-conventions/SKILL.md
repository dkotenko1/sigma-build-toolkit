...
description: >-
  Spec mechanics and conventions for Sigma workbooks and data models —
  element naming, page and folder layout, ID semantics on create-vs-update,
  secret handling, and the pitfalls that make generated Sigma JSON fail (or
  silently render nothing). Use when authoring, editing, reviewing or debugging a
  raw workbook spec. Its reference/ directory carries the verified spec shape for
  every element kind, the control catalog, formula-namespace rules, and the
  August 2026 breaking changes. Start here before writing any spec.
...
> `reference/workbook-spec-api.md`. Verified live against a live org 2026-08-07.
>
...
- `reference/command-center-recipes.md` — the shapes an executive overview page
  needs that the per-kind sections don't cover: the comparative KPI card
  (`comparisonColumn` + `comparison`), card sparklines, `progress` rings,
  row-scoped card grids via `MaxIf`, data-bound `text` elements, a live Cortex
  AI narrative, `region-map` with cross-filtering, control-driven dynamic
  charts, overlays/drill-through, and agent configuration. **Read this before
  building any overview or command-center page.**
- `examples/base-table-snowflake.sql` / `examples/base-table-databricks.sql` —
  **the one shared base table.** A runnable source table carrying the
  dimensions every control filters on plus the `Current Period` /
  `Prior Period` split that comparative KPIs need. Start any
  command-center build here; cross-filtering only works because every
  element shares one source.
- `reference/warehouse-portability.md` — what is portable across warehouses
  and what is not. **Read this if you are not on Snowflake**: the two places
  this repo assumes Snowflake (example seed SQL, and the AI-narrative
  function) with Databricks translations, plus write-back notes.
- `reference/api-cheatsheet.md` — condensed endpoint + element quick reference.
- `reference/openapi-is-source-of-truth.md` — query Sigma's published OpenAPI
  with `jq` rather than trusting any table in this repo.
