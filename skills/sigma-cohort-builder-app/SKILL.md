...
description: >-
  Build an interactive Sigma population-SEGMENTATION app from code
  (POST/PUT /v2/workbooks/spec): filter a population of individual records
  (customers, patients, employees, students, members, accounts) down to a named,
  saveable cohort, show live size/rate KPIs and distributions, then compare saved
  cohorts against each other. Trigger on "build a customer/patient/member
  cohort", "segment a population", "let an agent narrow a cohort by filters", or
  similar. This is population segmentation, NOT numeric projection under
  assumptions — for that use sigma-input-table-app. Encodes the verified tabbed-
  container, agent-tool-per-filter, wide-snapshot-save and grouped-table-sort
  shapes plus the hard limits, so it comes out right the first time.
...
engineered from a real production workbook (a production Marketing Control Center,
"Cohort Construction App" page) and verified end-to-end on staging. Pair with
pair it with a read-only dashboard page of your own, and see
...
1. Ask up front: what population is being segmented, what dimensions matter, and
   ...
   (in which case keep theming, logo and plugin choices consistent with
