...
- **`branded-dashboard-format`** — the standard house composition + a sample brand
...
## Your logo in the header — the recolour rule

A brand header is a gradient container with the company's real logo on it. Two
things make this go wrong, and both look like a broken asset rather than a
mistake:

**1. Never set `fill` on the `<svg>` root.** A root-level fill renders **BLACK**
in Sigma regardless of what you set it to. To put a dark wordmark onto a dark
header, recolour the **paths / class fills** individually:

```
<svg ...>                             <!-- no fill here -->
  <path d="..." fill="#FFFFFF"/>      <!-- recolour each path -->
  <path d="..." fill="#FFFFFF"/>
</svg>
```

If the mark is a single-colour raster rather than SVG, recolour the opaque
pixels and preserve the alpha channel — flattening alpha gives you a white box.

**2. A light surface needs its own recolour.** A white logo on a light-coloured
header or a printed report is invisible. Generate a second, dark variant rather
than reusing the white one; a helper that silently falls back to the white asset
will hand you a blank header and no error.

Many companies already publish an on-dark variant (often
`logo-white.svg` alongside `logo.svg`). Check for it before recolouring
anything — using their own asset is both faster and more accurate than your
recolour.

`scripts/fetch_logo.py <domain>` finds a company's published logo from their own
site and emits it as a `data:` URI ready to drop into an `image` element or a
container `backgroundImage`:

```bash
python3 scripts/fetch_logo.py example.com --datauri-file logo.txt
```

Use `--datauri-file`, not `--out`. `--out` writes the raw binary, and a spec
builder that does `read_text()` on that asset will corrupt it.

**Sample the palette from the logo itself** rather than guessing hex values — an
almost-right brand colour reads worse than an obviously neutral one.

If no logo can be found, say so out loud and use a neutral wordmark. Do not
hand-draw an approximation of someone's mark.
