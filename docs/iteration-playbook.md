...
Then grade the build against the **completeness checklist** in
`.claude/skills/sigma-workbook-conventions/reference/command-center-recipes.md`
§0 (or the modeler equivalent at the top of `sigma-input-table-app/SKILL.md`).
Everything above this line asks *is it broken?*; this asks *is it finished?* —
and the second question is the one a half-built dashboard passes. `validate-spec.py`
prints the same check as an advisory `[completeness]` line, naming the
interaction blocks (persona tabs, card grid, alert feed, drill-through,
navigation) the spec does not contain.
