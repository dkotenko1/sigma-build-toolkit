...
- All secrets live in `.env` (gitignored). The contract is three variables —
  `SIGMA_BASE_URL`, `SIGMA_CLIENT_ID`, `SIGMA_CLIENT_SECRET` — documented in
  README.md. No template file ships: credentials are per-organization, and a
  committed template is one careless edit away from being a committed secret.
